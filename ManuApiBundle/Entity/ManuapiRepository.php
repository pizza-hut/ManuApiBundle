...
/**
 * Class ManuapiRepository.
 */
class ManuapiRepository extends CommonRepository
{
    /**
    ...
    public function getEntities($args = [])
    {
        $q = $this->_em
            ...
            ->from('ManuApiBundle:Manuapi', $this->getTableAlias(), $this->getTableAlias().'.id');
    ...
    public function upCount($id, $type = 'sent', $increaseBy = 1)
    {
        try {
            ...
            $q->update(MAUTIC_TABLE_PREFIX.'manuapi_messages')
     ...
     * @return array
     */
    public function getManuapiList($search = '', $limit = 10, $start = 0, $viewOther = false, $manuapiType = null)
    {
        ...
        if (!empty($manuapiType)) {
            $q->andWhere(
                $q->expr()->eq('e.manuapiType', $q->expr()->literal($manuapiType))
            );
        }
